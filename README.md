...
**Status: protocol frozen, pre-data.** No corpus has been screened or coded. The article draft carries unfilled `[VERİ: ...]` placeholders throughout its findings sections. Only a two-article, 21-claim pilot exists, and that pilot is explicitly not protocol data.
...
**Freeze status: frozen 10 September 2026.** All conditions in §16 of the protocol were closed before the tag was applied. The frozen protocol is retrievable at `git show preregistration-v1:01_protocol_preregistration.md`, and the Zenodo record under the DOI above holds the same tree. Changes after this point are made in later commits and logged in the protocol change log and in Appendix D of the article.
